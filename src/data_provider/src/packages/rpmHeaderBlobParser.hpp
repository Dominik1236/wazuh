...
namespace RpmHeaderBlob
{
    ...
    static inline Package parse(const uint8_t* data, std::size_t size)
    {
        ...
        for (int64_t entry = 0; entry < indexSize; ++entry)
        {
            ...
            switch (tag)
            {
                case HEADER_TAG_NAME: fieldIndex = FIELD_NAME; break;

                case HEADER_TAG_ARCH: fieldIndex = FIELD_ARCHITECTURE; break;

                case HEADER_TAG_SUMMARY: fieldIndex = FIELD_DESCRIPTION; break;

                case HEADER_TAG_SIZE: fieldIndex = FIELD_SIZE; break;

                case HEADER_TAG_EPOCH: fieldIndex = FIELD_EPOCH; break;

                case HEADER_TAG_RELEASE: fieldIndex = FIELD_RELEASE; break;

                case HEADER_TAG_VERSION: fieldIndex = FIELD_VERSION; break;

                case HEADER_TAG_VENDOR: fieldIndex = FIELD_VENDOR; break;

                case HEADER_TAG_ITIME: fieldIndex = FIELD_INSTALL_TIME; break;

                case HEADER_TAG_GROUP: fieldIndex = FIELD_GROUP; break;

                case HEADER_TAG_SOURCERPM: fieldIndex = FIELD_SOURCE; break;
