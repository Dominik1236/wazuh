...
| [#38514](https://github.com/wazuh/wazuh/issues/38514) | Added zstd decompression of `/download` configuration responses: the agent advertises `Accept-Encoding: zstd` on config requests and transparently decompresses a `Content-Encoding: zstd` response before verifying `config_hash`, cutting the transferred bytes for shared configuration updates. |
